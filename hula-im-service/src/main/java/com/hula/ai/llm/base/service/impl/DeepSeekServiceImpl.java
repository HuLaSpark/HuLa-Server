...
import com.hula.ai.llm.deepseek.DeepSeekClient;
import com.hula.ai.llm.deepseek.DeepSeekStreamClient;
import com.hula.ai.llm.deepseek.enmus.Model;
import com.hula.ai.llm.deepseek.sse.SSEListener;
...
public class DeepSeekServiceImpl implements ModelService {
    ...
    public ChatMessageCommand chat(List<ChatMessageDTO> chatMessages, Boolean isDraw, Long chatId, String version) {
        if (ObjectUtil.isNull(deepSeekStreamClient)) {
            throw new BizException("DeepSeek无有效token，请切换其他模型进行聊天");
        }
        ...
        ChatCompletion chatCompletion = ChatCompletion.builder()
                ...
                .model(ObjectUtil.isNotNull(version) ? version : Model.CHAT.getName())
                .build();
    ...
    public Boolean streamChat(HttpServletResponse response, SseEmitter sseEmitter, List<ChatMessageDTO> chatMessages, Boolean isWs, Boolean isDraw,
        ...
        if (CollUtil.isEmpty(deepSeekStreamClient.getApiKey())) {
        ...
        SSEListener sseListener = new SSEListener(response, sseEmitter, chatId, conversationId, ChatModelEnum.DEEPSEEK.getValue(), version, uid.toString(), isWs);
