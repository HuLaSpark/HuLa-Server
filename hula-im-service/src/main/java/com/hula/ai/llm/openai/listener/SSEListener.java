package com.hula.ai.llm.openai.listener;
...
public class SSEListener extends EventSourceListener {
    ...
    private String handleOpenAI(String data) {
        ...
        if (ObjectUtil.isNotNull(completionResponse.getModel())) {
            version = completionResponse.getModel();
        }
        if (ObjectUtil.isNull(finishReason) && ObjectUtil.isNotNull(completionResponse.getChoices().get(0).getFinishReason())) {
